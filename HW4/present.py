...
def genRoundKeys(key):
    ...
    for i in range(1, FULLROUND + 2):
        ...
        # 64 leftmost bits of the current contents of register K
        roundkeys[i] = (key >> 16)
        
        # rotate 61 bit positions to the left
        key = rol(key,61,80)

        # pass the leftmost 4 bits through the S-box
        key = (sbox[key >> 76] << 76) + (key & (2 ** 76 - 1))

        #3. XOR bits [19:15] with round_counter, which is i
        key = key ^ (i << 15)
    return roundkeys
...
def sBoxLayer(state):
    ...
    # sixteen 4-bit words
    for i in range(16):
        # bit masking to keep only the last 4 bits of the shifted key.
        nibble = sbox[( state >> (i * 4)) & 0xF]
        # place the nibble to its corresponding position in the output 
        output += nibble << (i * 4)
    return output
...
def sBoxLayer_decrypt(state):
    ...
    for i in range(16):
        # bit masking to keep only the last 4 bits of the shifted key.
        nibble = sbox.index(( state >> (i * 4)) & 0xF)
        # place the nibble to its corresponding position in the output 
        output += nibble << (i * 4)
    return output
...
def pLayer(state):
    output = 0
    for i in range(64):
        original_bit = (state >> i) & 0x1
        new_pos = pmt[i]
        output += original_bit << new_pos
    return output
